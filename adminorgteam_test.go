...
func TestAdminOrgTeamListWithOptionalParams(t *testing.T) {
	...
	resp, err := client.Admin.Orgs.Teams.List(
		context.TODO(),
		"org-name",
		ngc.AdminOrgTeamListParams{
			PageNumber:        ngc.F(int64(0)),
			PageSize:          ngc.F(int64(0)),
			ScanAllowOverride: ngc.F(true),
			ScanByDefault:     ngc.F(true),
			ScanShowResults:   ngc.F(true),
			TeamName:          ngc.F("team-name"),
		},
	)
	if err != nil {
